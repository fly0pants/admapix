...
#   $env:ADMAPIX_API_KEY="<API_KEY>"; powershell -ExecutionPolicy Bypass -File install.ps1
#
param(
    [Parameter(Mandatory=$false)]
    [string]$ApiKey = $env:ADMAPIX_API_KEY
)

$ErrorActionPreference = "Stop"

if (-not $ApiKey) {
    Write-Host "用法: powershell -ExecutionPolicy Bypass -File install.ps1 -ApiKey <API_KEY>"
    Write-Host '  或: $env:ADMAPIX_API_KEY="<API_KEY>"; powershell -ExecutionPolicy Bypass -File install.ps1'
    exit 1
}
...
$SkillDir = Join-Path $env:USERPROFILE ".openclaw\skills\admapix"
