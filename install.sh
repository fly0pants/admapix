...
#   ADMAPIX_API_KEY=<API_KEY> bash install.sh
#
set -euo pipefail
...
API_KEY="${1:-${ADMAPIX_API_KEY:-}}"
if [ -z "$API_KEY" ]; then
  echo "用法: bash install.sh <API_KEY>"
  echo "  或: ADMAPIX_API_KEY=<API_KEY> bash install.sh"
...
SKILL_DIR="$HOME/.openclaw/skills/admapix"
if [ -d "$SCRIPT_DIR/skill" ]; then
  ...
  echo "  如需手动安装: cp -r skill/ ~/.openclaw/skills/admapix/"
fi
